...
**Comment identity:** `raiseOnBehalfOf` controls the JSM request reporter only. Jira records every synced comment as the account behind the managed ACOS Jira vendor credential; the JSM comment API accepts only the body and visibility, not an author override. Therefore, replacing Jason Vandeboom as the visible comment author requires rotating that managed credential to either Emily's account or—preferably—a dedicated Enablement Do-er service account with GEP Service Desk Agent and Add Comments permission. Verify the rotation with `jira:get-myself` before enabling auto-create, then create one labelled pilot comment. Existing Jira comments retain their original author and cannot be reassigned.
