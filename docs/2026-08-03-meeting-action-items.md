...
| --- | --- | --- |
| Jira Service Management create contract | Spark / ACOS: expose JSM customer-request creation plus service-desk, request-type, and required-field discovery; alternatively return sanitized upstream field errors. | **Blocked upstream.** Generic `create-issue` is available, but the GEP service-desk form contract is not. `JIRA_AUTOCREATE_ENABLED` stays `false`. |
| Jira follow-up sync | Brandon after the initial JSM path is verified: validate `update-issue`, `add-comment`, and participant payloads, then add an auditable sync outbox. | **Not started by design.** It depends on a successful initial Jira issue and verified managed endpoint contracts. |
...
| Deck delivery | Brandon + Enablement Ops: choose a validated production renderer or request an approved Google Slides creation path. | **Partial.** The app produces a review-ready storyboard and JSON source, not a production PPTX or Google Slides file. |
